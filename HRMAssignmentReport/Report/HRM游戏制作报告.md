...
<center>计48-经42 王鹏杰 学号：2024010860</center>
<center>计48-经42 高雅菁 学号：2024010858</center>
...
3. (**重要!!!**)运行前，需要将`mainwindow.cpp`程序第30，235，1597行的图片地址更新为本机机器人gif图片**绝对地址**，第761，1086，1240行的图片地址更新为本机烟花`gif`图片地址，并且**重新编译**!!! 注意：1.使用**相对地址无法正常读取**，2.预编译好的二进制文件无法读取`gif`文件。
...
程序可以选择文件位置，但是如果您希望进行自动读取存档(`autoload`)，您需要预先在代码中指定游戏文件的**绝对路径**

由于Qt对相对路径的支持似乎有障碍，我们已经在注意事项中提示您在`mainwidow.cpp`中对`gif`文件的路径进行修改，请确保您执行了修改。
...
弹出窗口，让用户打开`levelinfo.txt`文件(不要求文件名相同)，将输入流传递到`parselLevelInfo`函数中解析文本文件，将解析出的关卡信息储存在许多`vector`中，然后根据这些信息实例化`Game`，并`push`到`games`中。

**loadArchive 函数**

首先检测是否进行了`levelInfo`加载，如果没有加载，弹出报错信息。

如果已经加载，弹出窗口，让用户打开`archive.txt`文件(不要求文件名相同)，通过`parselArchive`函数解析文本文件，根据这个修改`games`中`Game`类的`passed`信息。
...
游戏的流程：玩家输入指令 $\rightarrow$ 游戏处理玩家输入 $\rightarrow$ 根据游玩过程展示状态 $\rightarrow$ 转到结算界面

可能遇到的错误

1. 无限循环 当`actualSteps>1e6`时，游戏认为这是一个无限循环，会直接结束，并显示`Infinite Loop` (注意：尽管`1e6`对于评测机来说是个很小的数字，但是扔谈需要一点时间，请不要立即点击`SKIP`)
2. 指令错误：这个包括多方面的错误，比如用了非法指令，拼写错误，以及参数错误。
3. 答案错误：正常执行完所有指令后输出结果与预期不匹配。

### 2. Success界面

![alt text](successwithfirework.png)

### 3. Fail界面

#### 3.1 Error

![alt text](failerror.png)
...
![alt text](playgamelevel4.png)

![alt text](playgamelevel5.png)
...
#### 2.4 成功范例
第四关最大武力值答案为$19233$，根据目测的成功的解决方案如下：
```
inbox
inbox
inbox
